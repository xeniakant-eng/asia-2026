...
function getExpenseInput(body: Record<string, unknown>) {
  ...
  const currency: ExpenseCurrency | "" = body.currency === "MAD" ? "MAD" : body.currency === "JPY" ? "JPY" : body.currency === "TWD" ? "TWD" : body.currency === "VND" ? "VND" : body.currency === "CAD" ? "CAD" : body.currency === "USD" ? "USD" : "";
  ...
  try {
    ...
    const response = await fetch(`${config.url}/rest/v1/trip_expenses`, {
      ...
      body: JSON.stringify({
        ...
        amount_local: currency === "MAD" || currency === "JPY" || currency === "TWD" || currency === "VND" ? amount : null,
