...
export default function TravelSite() {
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain p-4 text-sm leading-6 text-white/65 sm:p-6">
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              <p className="text-xs text-white/45">9 nights</p>
            </div>
            <div className="space-y-3">
              {[
          ...
          <section className="space-y-8">
            {taipeiItineraryDays.map((day) => (
              <DayArticle key={day.date} date={day.date} title={day.title}>
                {card(
                  <>
                    <p className="mb-4 text-sm text-white/50">{day.subtitle}</p>
  ...
  const openFirstAllowedMapChapter = (...chapters: PageName[]) => {
    ...
    if (chapter) openChapterPage(chapter);
  };
  return (
...
function SegmentButtons({ segments, onOpenSegment }: { segments: DashboardSegment[]; onOpenSegment: (page: PageName) => void }) {
  if (!segments.length) {
  ...
  return <div className="mt-5 space-y-3 text-sm leading-7 text-white/70"><p className="text-xs uppercase tracking-[0.22em] text-white/35">You are joining:</p>{segments.map((segment) => <button key={segment.label} type="button" disabled={segment.disabled} onClick={() => !segment.disabled && onOpenSegment(segment.page)} className={segment.disabled ? "w-full cursor-not-allowed rounded-2xl border border-white/10 bg-white/[0.025] px-4 py-3 text-left opacity-55" : "w-full rounded-2xl border px-4 py-3 text-left transition"} style={segment.disabled ? undefined : { borderColor: `${segment.color}44`, backgroundColor: `${segment.color}12` }}><p className="font-medium" style={{ color: segment.disabled ? "rgba(255,255,255,0.35)" : segment.color }}>{segment.label}</p></button>)}</div>;
}
